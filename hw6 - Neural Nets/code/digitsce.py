...
# Neural Net using Cross-Entropy Loss function
...
# Training Neural Net
before = time.time()
digits = nn.NeuralNet(784, 200, 10, 1)
digits.initialize_weights()
#digits.W1 = np.load('weights_ce_W1.npy')
#digits.W2 = np.load('weights_ce_W2.npy')
loss, acc = digits.trainNeuralNet(train_data, train_labels, 0.01, 1)
after = time.time()
print("TOOK " + str(after-before) + " SECONDS")

# Saving Weights to File
np.save('weights_ce_W1', digits.W1)
np.save('weights_ce_W2', digits.W2)
np.save('ce_acc', np.array(acc))
np.save('ce_loss', np.array(loss))
